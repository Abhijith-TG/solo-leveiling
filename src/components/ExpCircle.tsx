...
if (value <= 10) {
  ...
  return (
    <div className="bg-black sm:w-90 sm:h-90 w-65 h-65 flex items-center justify-center relative  shadow-[0_0_10px_rgba(168,85,247,0.8)] rounded-xl mb-3">
      <div className="absolute top-2 right-2">
        ...
        <p className="text-green-400 sm:text-base text-xs  absolute sm:bottom-17 bottom-24">
          <span className={`${color}`}>{user.xp}</span> / <span>{user.maxXp}</span>  XP
        </p>
        {/* <p onClick={()=> navigate('/acheivments')} className="text-green-600 text-base text-sm hover:text-green-400 hover:cursor-pointer absolute bottom-10">
          Acheivements
        </p> */}
      </div>
  )
