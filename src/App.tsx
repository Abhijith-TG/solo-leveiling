...
const strokeDashoffset =
  ...
  const increaseXp = (id:number)=>{
    ...
    setTasks((prev)=>prev.map(t=>t.id===task[0].id ? {...t,completed:true}:t))
    
    const newXp = user.xp+task[0].xp;

    if(newXp>=user.maxXp){
      setUser((prev)=>({level:prev.level+1,xp:newXp-prev.maxXp,maxXp:(prev.level+1)*100}))
      }else
...
<p className="text-green-400 text-2xl font-bold absolute">Level {user.level} </p>
<p className="text-green-400 text-md  absolute bottom-17">{user.xp+"/"+user.maxXp} XP</p>
    ...
    <div>
      {
        tasks.map((t)=>
          <div className="flex gap-5 my-2 w-90 justify-between" key={t.id}  >
            ...
            <button disabled={t.completed} onClick={()=>increaseXp(t.id)} className={`${t.completed ? "bg-gray-400":"bg-green-400 hover:cursor-pointer hover:bg-green-200"} px-4 w-30 py-1`}  >{t.completed ?"Completed":t.xp+" Gain"}</button>
          </div>
        )
      }
